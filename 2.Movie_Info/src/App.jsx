...
function Search({ onSearch }) {
  return (
    <input
      ...
      className='input input-bordered w-30 md:w-auto placeholder:text-xs placeholder:text-start'
...
function NavBar({ onSearch, totalResults }) {
  ...
  return (
    <div className='navbar bg-base-200 flex justify-evenly'>
      <div className=''>
        <a className='btn btn-ghost normal-case text-xl'>🎬The Movie App</a>
        <div className='form-control'>{<Search onSearch={onSearch} />}</div>
      </div>
      <div className='flex space-x-4'>
        <button className='btn btn-info rounded-full'>Found {totalResults} Results</button>
        ...
        <div className='text-[10px] text-slate-400'>(Built by Tushar D)</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className='card bg-base-100 shadow-xl flex w-full p-5'>
      {isLoading ? (
        ...
        <>
          <button className='btn w-max btn-accent' onClick={() => setShowWatchList(true)}>
            ⬅️ Show Watchlist
          </button>
          <figure>
          ...
          <div className='card-body'>
            <h2 className='card-title flex flex-col justify-start'>
              <div className='badge badge-accent badge-outline text-lg'>{title}</div>
              ...
              <div className='flex flex-row space-x-3 mt-2'>
                ...
                <div className='badge badge-error gap-2'>{`🕑Runtime: ${runtime}`}</div>
            ...
            <div className='mx-auto justify-center w-min'>
              <Stats imdbRating={imdbRating} imdbVotes={imdbVotes} boxOffice={boxOffice} />
            </div>
...
function MovieTable({ myWatchList, setMyWatchList, setImdbId, setShowWatchList }) {
  ...
  return (
    <div className='overflow-x-auto mt-3'>
      <table className='table'>
        ...
        <tbody>
          {myWatchList.map(({ Title, Poster, imdbRating, myRating, Runtime, imdbID }) => (
            <tr key={imdbID}>
              <td>
              ...
              <th>
                <button className='btn btn-error btn-xs' onClick={() => handleDeleteMovie(imdbID)}>
                  Delete
                </button>
              </th>
            </tr>
...
function MovieWatchList({ myWatchList, setMyWatchList, setShowWatchList, setImdbId }) {
  return (
    <div className='w-full h-full'>
      <button className='btn btn-accent m-5' onClick={() => setShowWatchList(false)}>
        ⬅️ Go Back to Info
      </button>
...
function App() {
  ...
  return (
    <div className='antialiased'>
      <NavBar Search={Search} onSearch={setSearchQuery} totalResults={movies.length} />
      <ShowMovies>
        <MovieResults
          query={searchQuery}
